...
class TSGMM(BaseClass.TBaseClass):
    ...
    MsgToCASM = None
    MsgToSOTM = None
    Etot = EnumTypes.FTotalEnergy()       # structure for total energy being minimized
    ...
    def LoadSettings(self):
        ...
        try:
            ...
            sett = open('settSGMM.ini')
                
            sett.readline() # [LogLevel]
            str = sett.readline() # value
            self.FLogLevel = int(str)
            
            sett.readline() # [DataPipeFName] <- input file
            str = sett.readline() # value
            str = str.strip('\n')
            self.HistFName = str
    ...
    def AnalyzeCycle(self):
        i = 0.0 # here the profit curves from SOTM (TDF file) is analyzed and variables are varied/adjusted
    
    def Poll(self):
        # this will be polling the RecMsg variable that contains an unprocessed message
        while True:
            # restarting cycle once SOTM finished
            if self.RecMsg == msg.PM_SOTM_READY:
